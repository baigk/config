...
class TokenFor : public Token {
 public:
  ...
  explicit TokenFor(std::string expr) {
    ...
    if (elements.size() != 4) {
        std::cout <<"cpp template string have error syntax 'for'" << ":" <<  expr << std::endl;
      exit(0);
    }
    m_val = elements[1];
    m_key = elements[3];

    cout << m_val << " " << m_key << endl;
  }
    ...
    if (text[0] == '$') {
      ...
      if (pos != std::string::npos) {
        ...
        //} else if (expression.find_first_of("if") == 0) {
          ...
          //cout << expression << "Dasd" << endl;
          tokens.push_back(std::shared_ptr<Token>(new TokenIf(expression)));
        //} else if (expression.find_first_of("iiii") == 0) {
        } else if (expression.find("set ") != expression.npos) {
           std::cout << "dad" << " " << expression << std::endl;
           tokens.push_back(std::shared_ptr<Token>(new TokenVarDef(expression))); 
        } else {
          //cout << expression << "Dasd88" << endl;
...
void parse_tree(token_vector* tokens,
  ...
  while (!tokens->empty()) {
    ...
    if (token->gettype() == TOKEN_TYPE_FOR) {
      ...
      parse_tree(tokens, &children, &define_vars, TOKEN_TYPE_ENDFOR);
      token->set_children(children);
      token->set_define_vars(define_vars);
    } else if (token->gettype() == TOKEN_TYPE_IF) {
      token_vector children, define_vars;
      parse_tree(tokens, &children, &define_vars, TOKEN_TYPE_ENDIF);
      token->set_children(children);
      token->set_define_vars(define_vars);
...
std::string parse(std::string templ_text, auto_data& data) {
  ...
  for (size_t i = 0 ; i < tree.size() ; ++i) {
    tree[i]->update_data(data);
    cout << i << " " << tree[i]->gettype() << endl;
