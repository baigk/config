...
bool KafkaProducer::init(const string &host, const int &port, const bool &async, const int &size)
{
	string errstr;
	this->topics_.clear();
	...
	if (async)
	{
		...
		conf_->set("queue.buffering.max.messages", to_string((long long)size).c_str(), errstr);
	}
	else
	{
	...
	producer_ = shared_ptr<RdKafka::Producer>(RdKafka::Producer::create(conf_.get(), errstr));
	if (!producer_) 
	{
...
int KafkaProducer::send(const char *data, const int &size, const string &topic, const int &partition, const int &timeout)
{
...
int KafkaProducer::send_batch(const vector<pair<const char *, const int &>> &data, 
	const string &topic, const int &partition, const int &timeout)
{
	return 0;
}

bool KafkaProducer::create_topic(const string &topic)
{
	string errstr;
	...
	shared_ptr<RdKafka::Topic> tpk = shared_ptr<RdKafka::Topic>(RdKafka::Topic::create(producer_.get(), topic, tconf_.get(), errstr));
	if (!tpk) 
	{
...
RdKafka::Topic *KafkaProducer::get_topic(const string &topic)
{
...
void KafkaProducer::close()
{
	...
	{
		cerr << "Waiting for " << producer_->outq_len() << endl;
		producer_->poll(1000);
	}
