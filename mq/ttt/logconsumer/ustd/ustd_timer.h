...
static string get_local_time(const char *format = "%Y-%m-%d %H:%M:%S", const time_t &gtime = time(NULL))
{
...
static string get_system_time(const char *format = "%a, %m %b %Y %H:%M:%S GMT", const time_t &gtime = time(NULL))
{
...
static time_t str2time(const string &date_time, const char *format = "%Y-%m-%d %H:%M:%S")
{
	tm tm_time;
