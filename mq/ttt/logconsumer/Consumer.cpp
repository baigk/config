...
bool Consumer::start()
{
	...
	for(auto it : topics_)
	{
		...
		threads_.push_back(thread(bind(&Consumer::worker, this, consumer_, it.first)));
	}
...
void Consumer::process_msg(const string& topic, const vector<char>& data)
{
	string file_name;
	...
	if (data.size() > SIZE_LEN)
	{
		...
		if (topic == "bid")
		{
			...
			file_name = uget_local_time(BID_FORMAT, bid.time());
		}
		else if(topic == "wn")
		{
			maxmob::rtb::Wn_t wn;
			wn.ParseFromArray((void*)&data[SIZE_LEN], size);
			file_name = uget_local_time(WN_FORMAT, wn.time());
		}
		else if(topic == "tc")
		{
			maxmob::rtb::Clk_t tc;
			tc.ParseFromArray((void*)&data[SIZE_LEN], size);
			file_name = uget_local_time(TC_FORMAT, tc.time());
		}
		else if(topic == "tm")
		{
			maxmob::rtb::Pv_t tm;
			tm.ParseFromArray((void*)&data[SIZE_LEN], size);
			file_name = uget_local_time(TM_FORMAT, tm.time());
		}
		else
		{
			return;
		...
		fstream out(get_full_name(log_path_, file_name), fstream::out | fstream::binary | fstream::app);
...
void Consumer::worker(uKafkaConsumer c, const string& topic)
{
	vector<char> data;
...
void Consumer::read_config(const char* path /*= "/etc/maxmob/logconsumer/consumer.conf"*/)
{
	...
	if (!(fp = fopen(path, "r"))) {
		cout<<"fail to open confsumer.conf"<<endl;
	}
	...
	while (fgets(buf, sizeof(buf)-1, fp)) {
		...
		if (!(t = strchr(b, '=')))
		{
			cout<<path<<":"<<line<<":"<<"expected name=value format"<<endl;
		}
		...
		*t = '\0';
		...
		{
			port_ = strlen(val) == 0 ? DEFAULT_PORT : stoi(val, NULL, 10);
		}
		else if(!strcmp(key, "log.interval"))
		{
			interval_ = strlen(val) == 0 ? DEFAULT_INTERVAL : stoi(val, NULL, 10);
		}
		else if(!strcmp(key, "log.count"))
		{
			count_ = strlen(val) == 0 ? DEFAULT_COUNT : stoi(val, NULL, 10);
		}
...
void Consumer::write_offset(const string& topic)
{
	int64_t offset;
	if(consumer_.get_last_offset(topic, offset))
	{
		fstream out(get_full_name(offset_path_, topic + ".offset"), fstream::out | fstream::binary | fstream::trunc);
		string str(to_string((long long)offset));
		out.write(str.c_str(), str.size());
		out.close();
	}
}

int64_t Consumer::read_offset(const string& topic)
{
	char buf[OFFSET_LEN] = {0};
	int64_t offset = RdKafka::Topic::OFFSET_BEGINNING;

	fstream in(get_full_name(offset_path_, topic + ".offset"), fstream::in | fstream::binary);
	in.read(buf, OFFSET_LEN);
	if(strlen(buf) > 0)
	{
		offset = stoll(buf, NULL, 10);
	}
	in.close();
...
string Consumer::get_full_name(const string& path, const string& file_name)
{
