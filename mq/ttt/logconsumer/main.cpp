...
static void kill_process(const vector<int>& pids)
{
	uConsumer::getInstance()->save(true);
...
{	
	if (argc > 1)
	{
		...
		usystem::get_process_id(process_name.c_str(), pids);
	...
	if (!uConsumer::getInstance()->start())
	{
		printf("logconsumer server start error!\n");
		return 0;
	}

	uConsumer::getInstance()->wait();
